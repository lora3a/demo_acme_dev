...
void read_vcc_vpanel(h10_adc_t *dev)
{
    ...
    int32_t vpanel = h10_adc_read_vcc(dev);
...
void send_lora_message(lora_state_t *lora, char *payload, size_t size)
{
...
void node_HT_print(const node_HT *node)
{
    ...
    printf(
        ...
        node->header.signature,
        node->header.s_class, cpu_id,
        node->header.vcc, node->header.vpanel,
...
void sensors_read(void)
{
    ...
    send_lora_message(&lora, payload, NODE_HT_SIZE);
}
