...
//
void binaryToJsonHeader(int16_t * rssi, int8_t *snr, uint16_t signature, uint8_t class, char * cpuid_hex, double vcc, double vpanel, char * json)
{
char strFmt[10];
...
uint16_t vpanel;
	...
	if(res)
	{
		...
		{
			printf("Cpu Id        : %s\n",cpuid_hex);
			printf("Voltage       : vcc = %5.3f, vpanel = %5.3f\n", ((double)vcc/1000.), ((double)vpanel/1000.));

			memset(strMsg,0,sizeof(strMsg));
			sprintf(strMsg,"{");
			binaryToJsonHeader(rssi, snr, signature, s_class, cpuid_hex, ((double)vcc/1000.), ((double)vpanel/1000.), strMsg);
			switch(s_class)
			{
				case NODE_HT_CLASS :
